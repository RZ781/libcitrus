...
CitrusVector CitrusVector_rotate_clockwise(CitrusVector a)
{
	return (CitrusVector) {
	a.y, -a.x};
}
...
void CitrusGame_lock_piece(CitrusGame *game)
{
	...
	if (game->current_piece == &citrus_pieces[CITRUS_COLOR_T]
		...
		// count number of corners filled in on front and back of piece
		int corners[2] = { 0, 0 };
		CitrusVector center =
	...
	int score;
	if (spin) {
		score = game->config.t_spin_scores[cleared_lines];
	} else if (mini_spin) {
		score = game->config.mini_t_spin_scores[cleared_lines];
	} else {
...
// apply soft drop and zero arr, which needs to be done every tick and keypress
void CitrusGame_constant_movement(CitrusGame *game)
{
	if (game->move_direction != 0 && game->config.arr == 0) {
